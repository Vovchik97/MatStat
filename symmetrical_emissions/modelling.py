...
from estimations import Var, Mean, Median
...
class Modelling(ABC):
    ...
    С синхронными выбросами
    
    def __init__(self, sample: list, truth_value: float, n: int):
        self.bootstrap = Bootstrap(sample, n)
        self.truth_value = truth_value
        self.estimation_list = self.bootstrap.bootstrap_run()

    def estimate_bias_sqr(self, index: int):
        return (Mean().estimate(self.estimation_list[index]) - self.truth_value) ** 2

    def estimate_var(self, index: int):
        return Var().estimate(self.estimation_list[index])

    def estimate_mse(self):
        return self.estimate_bias_sqr(0) + self.estimate_var(0), self.estimate_bias_sqr(1) + self.estimate_var(1)

    def run_model(self):
        return self.estimate_mse()

    def get_samples(self):
        return self.estimation_list
"""